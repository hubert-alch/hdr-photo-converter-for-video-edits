...
cleanup_sensitive_material() {
    if [[ -n "$NOTARY_TEMP_DIR" ]]; then
        rm -rf "$NOTARY_TEMP_DIR"
    fi
}
trap cleanup_sensitive_material EXIT
...
configure_notarization() {
    ...
    if [[ -n "${NOTARYTOOL_PROFILE:-}" ]]; then
        ...
        NOTARY_TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hdr-photo-converter-notary.XXXXXX")"
        NOTARY_API_KEY_PATH="$NOTARY_TEMP_DIR/AuthKey_$APPLE_API_KEY_ID.p8"
        printf '%s' "$APPLE_API_KEY_BASE64" | base64 --decode > "$NOTARY_API_KEY_PATH"
        chmod 600 "$NOTARY_API_KEY_PATH"
        notary_args+=(--key "$NOTARY_API_KEY_PATH" --key-id "$APPLE_API_KEY_ID" --issuer "$APPLE_API_ISSUER_ID")
    else
        echo "NOTARIZE=1 requires a notarytool keychain profile or an App Store Connect team API key." >&2
        exit 1
    fi
}
...
if [[ -n "$SIGNING_IDENTITY" && "$SIGNING_IDENTITY" != "-" ]]; then
    ...
    if [[ "$REQUIRE_DISTRIBUTION_SIGNING" == "1" ]]; then
        echo "A Developer ID signing identity is required for distribution." >&2
        exit 1
    fi
...
if [[ -n "$SIGNING_IDENTITY" && "$SIGNING_IDENTITY" != "-" ]]; then
    actual_team_id="$(codesign -dvv "$APP_BUNDLE" 2>&1 | awk -F= '/^TeamIdentifier=/{ print $2; exit }')"
    if [[ "$actual_team_id" != "$TEAM_ID" ]]; then
        echo "Signed app has TeamIdentifier '$actual_team_id'; expected '$TEAM_ID'." >&2
        exit 1
    fi
fi
...
if [[ "$NOTARIZE" == "1" ]]; then
    NOTARY_TEMP_DIR="${NOTARY_TEMP_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/hdr-photo-converter-notary.XXXXXX")}"
    app_notary_zip="$NOTARY_TEMP_DIR/app-notary.zip"
